...
from input import load_images_from_folder_rgb, create_image_folder_dataset
import pandas as pd
...
from transformers import ViTFeatureExtractor, DefaultDataCollator, create_optimizer, TFViTModel
...
rocks360_ds = create_image_folder_dataset("360 Split")
...
classification_layer = tf.keras.layers.Dense(3, activation='softmax',name='classification')
...
mds_model.save('mds_10_'+str(num_train_epochs)+'_'+str(train_batch_size)+'_'+str(eval_batch_size)+'_'+str(learning_rate)+'_'+str(weight_decay_rate)+'_'+str(num_warmup_steps)+'_'+str(num_dense_layers)+'_'+'.keras')
